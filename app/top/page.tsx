...
export default function Top() {
  const [ideas, setIdeas] = useState<FireIdea[]>([]);
  ...
  useEffect(() => {
    const unsub1 = onSnapshot(collection(db, "ideas"), (snap) => {
      setIdeas(snap.docs.map((d) => ({ id: d.id, ...(d.data() as any) })));
    });
  ...
  const ranked = useMemo(() => [...enriched].sort((a, b) => (b.score || 0) - (a.score || 0)), [enriched]);
  const latest = useMemo(
    () =>
      [...enriched].sort((a, b) => {
        const aTime = new Date((a.createdAt as any) || a.publishedAt || 0).getTime();
        const bTime = new Date((b.createdAt as any) || b.publishedAt || 0).getTime();
        return bTime - aTime;
      }),
    [enriched]
  );
              ...
              <div className="flex flex-col md:flex-row gap-4">
                <div className="md:w-64 md:h-40 w-full h-40 rounded-xl overflow-hidden border bg-zinc-50">
                  {hero.sourceURL || hero.sourceUrl ? (
                    <img
                      src={`/api/og?url=${encodeURIComponent((hero.sourceURL as string) || (hero.sourceUrl as string))}`}
                      alt="hero"
                  ...
                  {(hero.tags || []).length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-2 text-xs">
                      {(hero.tags || []).slice(0, 6).map((t) => (
                        <span key={t} className="rounded-full bg-zinc-100 px-2 py-0.5 border border-zinc-200">
                          {t}
                        </span>
                      ))}
                    </div>
              ...
              {trending.map((i) => (
                <IdeaCard
                  ...
                  idea={{
                    id: i.id,
                    title: i.title,
                    sourceUrl: (i.sourceURL as string) || (i.sourceUrl as string),
